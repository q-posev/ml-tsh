...
do
  #add=100
  #up=$(($i+$add-1))
  mkdir $TMP_DEMON/$(($i-1)).ml
  cp 500-inps/init_$i/geom-phen.xyz $TMP_DEMON/$(($i-1)).ml/
  cp 500-inps/init_$i/velo $TMP_DEMON/$(($i-1)).ml/
  cp do-dftb-zn-mod.py $TMP_DEMON/$(($i-1)).ml/
done
